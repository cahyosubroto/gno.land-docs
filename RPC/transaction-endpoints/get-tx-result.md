...
excerpt: ""
hidden: false
createdAt: "Fri Aug 23 2024 02:20:43 GMT+0000 (Coordinated Universal Time)"
updatedAt: "Fri Aug 23 2024 09:45:30 GMT+0000 (Coordinated Universal Time)"
---
The `getTxResult` endpoint retrieves the result of a specific transaction from the blockchain.

## Example Request

In the request example, we need to specify the transaction's hash that we want to retrieve in the `params` field.
...
If no transaction result is found or not yet indexed in the blockchain, the request will return the following response:
...
| `params`     | `Array`   | An array containing the transaction's hash (e.g., `"AP9YX+QXrIByqonIqStod8G9EI5AMiUZhsXk58wr0ws="`). |

### Response Arguments

| **Argument** | **Type**  | **Description**                                                                    |
| ------------ | --------- | ---------------------------------------------------------------------------------- |
| `result`     | `String`  | The Base64 encoded, Amino encoded binary data representing the transaction result. |
